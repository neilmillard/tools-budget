...
describe('AffordabilityCalculator', () => {
    ...
    test('renders the calculator with default values', () => {
        ...
        expect(screen.getByText('Mortgage Affordability Calculator')).toBeInTheDocument();
        ...
        expect(screen.getByLabelText('Projected Interest Rate: 5%')).toBeInTheDocument();
    });
    ...
    test('calculates affordability with all inputs filled', async () => {
        ...
        fireEvent.change(screen.getByPlaceholderText('e.g. 50000'), { target: { value: '60000' } });
        fireEvent.change(screen.getByPlaceholderText('e.g. 25000'), { target: { value: '20000' } });
        fireEvent.change(screen.getByPlaceholderText('e.g. 500'), { target: { value: '1000' } });

        // Change interest rate via mock slider
        const interestSlider = screen.getByLabelText('Projected Interest Rate: 5%');
    ...
    test('shows "Too Many Expenses" when expenses exceed income', async () => {
        ...
        fireEvent.change(screen.getByPlaceholderText('e.g. 50000'), { target: { value: '24000' } });
        fireEvent.change(screen.getByPlaceholderText('e.g. 500'), { target: { value: '2500' } });
    ...
    test('applies maxed styling when monthly payment exceeds limit', async () => {
        ...
        fireEvent.change(screen.getByPlaceholderText('e.g. 50000'), { target: { value: '48000' } });
        ...
        await waitFor(() => {
            ...
            expect(paymentElement).toHaveClass('bg-red-100');
            expect(paymentElement).toHaveClass('text-red-700');
        });
    });
    ...
    test('updates calculation when interest rate changes', async () => {
        ...
        await waitFor(() => {
            const paymentText = screen.getByText(/Estimated Monthly Payment: £2,044.58/);
